...
struct AppData {
    ...
    goose::graphics::RenderData render_data;
};
...
init(AppData *data)
{
    ...
    if (!goose::graphics::create_instance(&data->render_data, "Sandbox", VK_MAKE_VERSION(0, 0, 1)))
    {
    ...
    if (!SDL_Vulkan_CreateSurface(data->window, data->render_data.instance, nullptr, &surface))
    {
        LOG_ERROR("{}", SDL_GetError());
        return false;
    }
...
{
    goose::graphics::cleanup(&data->render_data);
